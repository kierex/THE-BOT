/**
 * trivia.js — Trivia Quiz Game (with game database)
 * Sagutin ang trivia at manalo ng coins!
 * TEAM STARTCOPE BETA
 */
...
const QUESTIONS = [
    ...
    { q: 'Pinakamalaking kontinente sa mundo?', a: ['asia'], hint: 'Kasama Pilipinas' },
    { q: 'Speed of light (km/s approximate)?', a: ['300000','300,000'], hint: '3 x 10^5' },
    { q: 'Sino ang unang tao sa buwan?', a: ['neil armstrong','armstrong'], hint: 'Apollo 11, 1969' },
];
...
module.exports.config = {
    ...
    description:     'Trivia Quiz! Sagutin ang tanong at manalo ng coins!',
...
module.exports.run = async function ({ api, event, args }) {
    ...
    if (activeGames.has(`${threadID}_${senderID}`)) {
        ...
        if (args[0]?.toLowerCase() === 'hint')
            return api.sendMessage(`💡 ${bold('Hint:')} ${g.hint}`, threadID, messageID);
        return api.sendMessage(
    ...
    api.sendMessage(
        ...
        `🏆 Correct: +${WIN_COINS} | ❌ Wrong: ${LOSS_COINS}\n\n` +
        `❓ ${bold('Tanong:')}\n${q.q}\n\n` +
        ...
        (err, info) => {
            ...
            const timeout = setTimeout(() => {
                ...
                api.sendMessage(
                    ...
                    `💸 ${LOSS_COINS} coins → ${bold(newCoins.toLocaleString())} coins`,
                    threadID
                );
            }, tl * 1000);
            activeGames.set(`${threadID}_${senderID}`, { q, timeout, hint: q.hint });
            global.client.handleReply.push({
                name: 'trivia', messageID: info.messageID,
                author: senderID, answers: q.a, question: q.q, hint: q.hint, timeoutRef: timeout
            });
        },
        messageID
    );
...
module.exports.handleReply = async function ({ api, event, handleReply }) {
    ...
    activeGames.delete(`${threadID}_${senderID}`);
    const isCorrect = handleReply.answers.some(a => body.trim().toLowerCase() === a.toLowerCase());
    if (isCorrect) {
        ...
        return api.sendMessage(
            ...
            `💰 +${WIN_COINS} coins → ${bold(newCoins.toLocaleString())} coins\n\n💡 ${global.config.PREFIX}rich — leaderboard`,
            threadID, messageID
        );
    } else {
        gdb.recordResult(senderID, 'loss');
