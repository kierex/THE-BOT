  ...
  return async function ({ event }) {
    ...
    try {
      ...
      if (event.isGroup && !threadExists) {
        ...
        for (const singleData of threadIn4.userInfo) {
          if (singleData.gender !== undefined) {
            ...
            if (!allUserID.includes(String(singleData.id)) || !userName.has(String(singleData.id)) || !(await Users.getData(singleData.id))) {
              await Users.createData(singleData.id, { name: singleData.name, gender: singleData.gender });
        ...
        logger(`New thread: ${threadID} | ${threadIn4.threadName}`, '[ DATABASE ] >');
      }
      ...
      if (!userExists) {
        ...
        const setting3 = { name: infoUsers.name, gender: infoUsers.gender };
